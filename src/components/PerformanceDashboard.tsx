...
export function PerformanceDashboard({ holdings, totalValue, totalInvestment }: PerformanceDashboardProps) {
  ...
  const getDiversificationLabel = (score: number) => {
    ...
    if (score >= 60) return { label: 'İyi', color: 'text-accent-600 dark:text-accent-400' };
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
      <div className="group relative bg-gradient-to-br from-brand-50 via-brand-50 to-brand-100/70 dark:from-brand-900/25 dark:via-brand-900/20 dark:to-brand-800/15 rounded-2xl border border-brand-200/60 dark:border-brand-500/30 p-5 shadow-sm hover:shadow-lg hover:scale-[1.02] transition-all duration-300 overflow-hidden">
        <div className="absolute top-0 right-0 w-20 h-20 bg-brand-500/10 rounded-full blur-2xl group-hover:bg-brand-500/20 transition-all"></div>
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-3">
            <div className="p-2 bg-brand-100 dark:bg-brand-800/40 rounded-xl">
              <BarChart2 size={18} className="text-brand-600 dark:text-brand-400" />
            </div>
            <span className="text-xs font-bold bg-brand-100 dark:bg-brand-800/40 text-brand-700 dark:text-brand-300 px-2 py-1 rounded-full">
              Toplam
            </span>
          </div>
          <p className="text-xs font-bold uppercase tracking-wider text-brand-600/80 dark:text-brand-400/80 mb-1">Toplam Getiri</p>
          <p className={`text-3xl font-black leading-none mb-2 ${totalPnLPercent >= 0 ? 'text-brand-700 dark:text-brand-200' : 'text-red-600 dark:text-red-400'}`}>
      ...
      <div className="group relative bg-gradient-to-br from-green-50 via-accent-50 to-green-100/70 dark:from-green-900/25 dark:via-green-900/20 dark:to-accent-800/15 rounded-2xl border border-green-200/60 dark:border-green-500/30 p-5 shadow-sm hover:shadow-lg hover:scale-[1.02] transition-all duration-300 overflow-hidden">
      ...
      <div className="group relative bg-gradient-to-br from-red-50 via-brand-50 to-red-100/70 dark:from-red-900/25 dark:via-red-900/20 dark:to-brand-800/15 rounded-2xl border border-red-200/60 dark:border-red-500/30 p-5 shadow-sm hover:shadow-lg hover:scale-[1.02] transition-all duration-300 overflow-hidden">
      ...
      <div className="group relative bg-gradient-to-br from-accent-50 via-brand-50 to-accent-100/70 dark:from-accent-900/25 dark:via-accent-900/20 dark:to-brand-800/15 rounded-2xl border border-accent-200/60 dark:border-accent-500/30 p-5 shadow-sm hover:shadow-lg hover:scale-[1.02] transition-all duration-300 overflow-hidden">
        <div className="absolute top-0 right-0 w-20 h-20 bg-accent-500/10 rounded-full blur-2xl group-hover:bg-accent-500/20 transition-all"></div>
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-3">
            <div className="p-2 bg-accent-100 dark:bg-accent-800/40 rounded-xl">
              {diversificationScore >= 50
                ? <ShieldCheck size={18} className="text-accent-600 dark:text-accent-400" />
                : <ShieldAlert size={18} className="text-yellow-600 dark:text-yellow-400" />
              }
            </div>
            <span className="text-xs font-bold bg-accent-100 dark:bg-accent-800/40 text-accent-700 dark:text-accent-300 px-2 py-1 rounded-full">
              Skor
            </span>
          </div>
          <p className="text-xs font-bold uppercase tracking-wider text-accent-600/80 dark:text-accent-400/80 mb-1">Çeşitlendirme</p>
